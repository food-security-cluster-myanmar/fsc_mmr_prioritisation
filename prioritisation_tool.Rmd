...
fsc %>% 
  ...
  mutate(reached_pc = beneficiaries / fs_targeted,
         ...
         fs_targeted = round(fs_targeted, digits = 0)) %>% 
  arrange(desc(reached_pc)) %>% 
  mutate(has_beneficaries = ifelse(beneficiaries > 0, 1, 0)) %>% 
  group_by(group) %>% 
  summarise(beneficiaries = sum(beneficiaries),
            fs_targeted = sum(fs_targeted), 
            townships = sum(has_beneficaries),
            tsp_total = n()) %>% 
  mutate(pc_reached = round(beneficiaries / fs_targeted * 100, digits = 2), 
         ...
         pc_tsp = round(townships / tsp_total * 100, digits = 2)) %>% 
  select(group, beneficiaries, `%_beneficiaries` = pc_beneficiaries, 
         tsp_reached = townships, tsp_total,
         target = fs_targeted, `%_reached` = pc_reached) %>% 
