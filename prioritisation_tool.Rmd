...
### Establishing a beneficiary profile

```{r comparison-demographic-environmental}

survey %>% 
  select(matches("shocks_|hoh_female|rural|expense_food|not_improved|agri_activity|income_ms_|size|children|disability"), survey_id) %>% 
  ...
  left_join(survey %>% select(hoh_education, hoh_age, residence_type, survey_id) %>% 
              ...
              pivot_wider(names_from = var, values_from = value, values_fill = 0), by = "survey_id") %>%  
  pivot_longer(cols = -survey_id, names_to = "var", values_to = "value") %>% 
  left_join(survey %>% select(survey_id, priority), by = "survey_id") %>% 
  mutate(priority = recode(priority, `0` = "not_priority", `1` = "priority")) %>% 
  group_by(priority, var) %>% 
  summarise(mean = mean(value, na.rm = TRUE)) %>% 
  filter(var != "NA") %>% 
  pivot_wider(names_from = priority, values_from = mean, values_fill = 0) %>% 
  mutate(diff_pc = round((priority - not_priority) / not_priority * 100, digits = 2)) %>% 
  arrange(desc(diff_pc)) %>% 
  left_join(
    survey %>%
      select(matches("shocks_|hoh_female|rural|expense_food|not_improved|agri_activity|income_ms_|size|children|disability"), survey_id) %>% 
      select(-shocks_none, -crp_landsize) %>% 
      left_join(survey %>% select(hoh_education, hoh_age, residence_type, survey_id) %>% 
                  pivot_longer(cols = -survey_id, names_to = "old_var", values_to = "var") %>%
                  mutate(value = 1) %>% 
                  select(-old_var) %>% 
      ...
      summarise(sum = sum(value, na.rm = TRUE), 
                count = n()) %>% 
      mutate(proportion = round(sum / count * 100, digits = 2)) %>% 
      select(-sum, -count) %>% 
      filter(var != "NA"), by = "var"
  ) %>% 
  mutate(proportion = case_when(var == "children_0_4" ~ round(938 / 2708 * 100, digits = 2), 
                                var == "hh_size" ~ 5.10, 
                                TRUE ~ proportion),
         priority = round(priority * 100, digits = 2), 
         not_priority = round(not_priority * 100, digits = 2)) %>% 
  rename(`%_sample` = proportion, `%_diff` = diff_pc, variable = var) %>% 
  datatable(filter = list(position = "top", clear = FALSE),
            options = list(pageLength = 15, scrollX = TRUE),
            caption = htmltools::tags$caption(style = "caption-side: top;
                                              text-align: center; font-size: 130%;",
                                              "Components of the beneficiary profile")) %>% # rewrite this when you're not tired
  formatStyle(0, target = "row", lineHeight = "80%", fontSize = "80%")
  
# i kinda hate having to do it this way, but i don't have time
# survey %>% 
#   mutate(has_children_0_4 = ifelse(children_0_4 > 0, 1, 0)) %>% 
#   {sum(.$has_children_0_4)}
# 
# mean(survey$hh_size, na.rm = TRUE)
...
vul_share <- vulmmr %>% select(avg_safe_sanitation_improved_drinking_water:female_literacy_percent_literate, 
  ...
  mdepriv(c("avg_safe_sanitation_improved_drinking_water", "average_good_roof_and_wall", 
            ...
            "child_dependency_ratio_31", "unpaid_family_worker_percent", "conflict_score"), 
          method = "cz", output = "all", 
          score_i_heading = "vul_score")
  
  
glimpse(vul_share)
vul_share$summary_by_item

vulmmr %>% 
  select(avg_safe_sanitation_improved_drinking_water:female_literacy_percent_literate,
         child_dependency_ratio_31, unpaid_family_worker_percent,
         admin3_pcode = township_pcode) %>% 
  left_join(conflict_score %>% select(conflict_score = score_i, admin3_pcode), by = "admin3_pcode") %>% 
  select(-conflict_index_c_percent_of_max_inverse_percent_of_average_of_envelopes_12, 
         -unpaid_family_workers_inverse, -child_dependency_ratio_inverse) %>% 
  relocate(admin3_pcode, .after = conflict_score) %>% 
  filter(!is.na(admin3_pcode)) %>% 
  mutate_at(vars(avg_safe_sanitation_improved_drinking_water:female_literacy_percent_literate), ~ 1 - .x) %>% 
  replace(is.na(.), 0) %>% 
  rowwise() %>% 
  mutate(vul_score_flat = mean(c(avg_safe_sanitation_improved_drinking_water:conflict_score))) %>% 
  left_join(vul_share$data %>% select(admin3_pcode, vul_score), by = "admin3_pcode") %>% 
  ggplot(aes(x = vul_score_flat, y = vul_score)) + 
  geom_point()
